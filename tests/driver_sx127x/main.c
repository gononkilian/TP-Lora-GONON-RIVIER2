...
int send_cmd(int argc, char **argv)
{
    ...
    payload_struct pld ;
    pld.to = (uint8_t)strtoul(argv[2], NULL, 0);
    pld.from = 13;
    pld.hop=0;
    pld.hoplim=3;
    pld.role=0x1;
    pld.msg_lenght=(unsigned)strlen(argv[1]) ;
    
    memcpy(pld.message,argv[1],(unsigned)strlen(argv[1]));
   
    if (strlen(argv[1])==0){
    	pld.msg_type=0x1;
    	pld.msg_lenght=0;
    	memcpy(pld.message,"",0);
    }
    else pld.msg_type=0x2;
    printf("sending \"%s\" payload (%u bytes)\n",
           argv[1], (unsigned)strlen(argv[1]) +10 +1);

    iolist_t iolist = {
        .iol_base = &pld,
        .iol_len = (strlen(argv[1]) + 1+10)
    };
...
static void _event_cb(netdev_t *dev, netdev_event_t event)
{
    if (event == NETDEV_EVENT_ISR) {
		...
		memcpy(&pld, message,sizeof(payload_struct));
		
		
		printf(
		    "to:%02X, from:%02X, hop:%02X, hoplim:%02X, role:%02X, msg_type:%02X, msg_length:%08lX\n message:%s\n",
		    pld.to,
		    pld.from,
		    pld.hop,
		    pld.hoplim,
		    pld.role,
		    pld.msg_type,
		    (unsigned long)pld.msg_lenght,
		    pld.message
		);
		if (pld.to == 13){
			printf(
		    "MESSAGE reçut de la part de %02X:%s\n"
		    ,pld.from,pld.message);
		}
		else{
...
int init_sx1272_cmd(int argc, char **argv)
{
    ...
    if (netdev->driver->init(netdev) < 0) {
        puts("Failed to initialize SX127x device");
        return 1;
    }
    ...
    _recv_pid = thread_create(stack, sizeof(stack),
